...
namespace DOCUMENTSERVICE.Controllers
{
    ...
    {
        [HttpGet]
        [Route("api/Portal/Pension/Corporate/policy_documents/{policy_number}")]
        public IHttpActionResult GetPensionCorporatePolicyDocuments(string policy_number)
        {
                ...
                try
                {
                    ...
                    if (bbConnected)
                    {
                        using (ClientContext ctx = new ClientContext(Constants.SharepointURL))
                        {
                            ...
                            var parentFolderName = Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sCorperate + "/" + PolicyNumber + "/";
        ...
        [HttpGet]
        [Route("api/Portal/Pension/Personal/policy_documents/{policy_number}")]
        public IHttpActionResult GetPensionPersonalPolicyDocuments(string policy_number)
        {
            List<PolicyFiles> response = new List<PolicyFiles>();
            string PolicyNumber = policy_number;
            if (!string.IsNullOrEmpty(PolicyNumber))
            {
                ...
                try
                {
                    ...
                    if (bbConnected)
                    {
                        ...
                        {
                            var secret = new SecureString();
                            var parentFolderName = Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sPersonal + "/" + PolicyNumber + "/";
                            foreach (char c in Constants.SharepointPassword)
                            {
                                secret.AppendChar(c);
                            }
        ...
        [HttpGet]
        [Route("api/Portal/Life/Personal/policy_documents/{policy_number}")]
        public IHttpActionResult GetLifePersonalPolicyDocuments(string policy_number)
        {
            List<PolicyFiles> response = new List<PolicyFiles>();
            string PolicyNumber = policy_number;
            if (!string.IsNullOrEmpty(PolicyNumber))
            {
                ...
                try
                {
                    ...
                    if (bbConnected)
                    {
                        ...
                        {
                            var secret = new SecureString();
                            var parentFolderName = Constants.SharepointLibraryURI + "/" + Constants.sLife + "/" + Constants.sPortal + "/" + Constants.sPersonal + "/" + PolicyNumber + "/";
                            foreach (char c in Constants.SharepointPassword)
                            {
                                secret.AppendChar(c);
                            }
        ...
        /// </summary>
        [HttpGet]
        [Route("api/Portal/document/{file_name}")]
        ...
        /// </summary>
        [HttpGet]
        [Route("api/Portal/documentview/{file_name}")]
        ...
        [HttpGet]
        [Route("api/Portal/GetFileBase64String/file_name")]
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("api/Portal/Pension/Personal/upload")]
        public IHttpActionResult UploadPensionPersonalDocument()
        {
            string error = string.Empty;
                ...
                if (filedetails.policyNo != null)
                {
                    if (filedetails != null && filedetails.file.ContentLength > 0)
                    {
                        ...
                        if (bbConnected)
                        {
                            ...
                            var sDocName = UploadPensionPersonalDocuments(filestream, filedetails.docName, sSpSiteRelativeUrl, Constants.SharepointLibrary, filedetails.policyNo, fileinfor, filedetails.docName, filesize);
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("api/Portal/Pension/Corporate/upload")]
        public IHttpActionResult UploadPensionCorporateDocument()
        {
            string error = string.Empty;
            HttpRequestMessage request = this.Request;
            if (!request.Content.IsMimeMultipartContent())
                ...
                if (filedetails.policyNo != null)
                {
                    if (filedetails != null && filedetails.file.ContentLength > 0)
                    {
                        ...
                        if (bbConnected)
                        {
                            ...
                            long fileSizeGBS = filesize / (1024);
                            Stream filestream = new MemoryStream(filedetails.file.ContentLength);
                            var sDocName = UploadPensionCorporateDocuments(filestream, filedetails.docName, sSpSiteRelativeUrl, Constants.SharepointLibrary, filedetails.policyNo, fileinfor, filedetails.docName, filesize);
                            ...
                            using (ClientContext ctx = new ClientContext(Constants.SharepointURL))
                            {
                                ...
                                try
                                {
                                    ...
                                    ctx.Load(ctx.Web);
                                    ctx.ExecuteQuery();
                                    var FolderRelativeURL = Constants.SharepointURL + Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sCorperate + "/" + filedetails.policyNo + "/" + filedetails.docName;
                                    Uri fileUri = new Uri(FolderRelativeURL);
                                    Microsoft.SharePoint.Client.File file = ctx.Web.GetFileByServerRelativeUrl(fileUri.AbsolutePath);
                                    ctx.Load(file);
                                    ctx.ExecuteQuery();
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("api/Portal/Pension/Personal/UploadDocumentString")]
        public HttpResponseMessage UploadPensionPersonalDocumentString([FromBody]JToken postData, HttpRequestMessage request)
        {
            // Initialization  
                ...
                if (filedetails.policyNo != null)
                {
                    if (filedetails != null && filedetails.file.Length > 0)
                    {
                        ...
                        if (bbConnected)
                        {
                            ...
                            var sDocName = UploadPensionPersonalDocuments(filestream, fileName, sSpSiteRelativeUrl, Constants.SharepointLibrary, filedetails.policyNo, fileinfor, filedetails.fileExtension, 0);
                            ...
                            using (ClientContext ctx = new ClientContext(Constants.SharepointURL))
                            {
                                ...
                                try
                                {
                                    ...
                                    ctx.Load(ctx.Web);
                                    ctx.ExecuteQuery();
                                    var FolderRelativeURL = Constants.SharepointURL + Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sPersonal + "/" + filedetails.policyNo + "/" + fileName;
                                    Uri fileUri = new Uri(FolderRelativeURL);
                                    Microsoft.SharePoint.Client.File file = ctx.Web.GetFileByServerRelativeUrl(fileUri.AbsolutePath);
                                    ctx.Load(file);
                                    ctx.ExecuteQuery();
        ...
        [HttpPost]
        [Route("api/Portal/Pension/Corporate/UploadDocumentString")]
        public HttpResponseMessage UploadPensionCorporateDocumentString([FromBody]JToken postData, HttpRequestMessage request)
        {
            // Initialization  
            HttpResponseMessage response = null;
            UploadFileStringModel requestObj = JsonConvert.DeserializeObject<UploadFileStringModel>(postData.ToString());
                ...
                if (filedetails.policyNo != null)
                {
                    if (filedetails != null && filedetails.file.Length > 0)
                    {
                        ...
                        if (bbConnected)
                        {
                            ...
                            byte[] bytes = Convert.FromBase64String(filedetails.file);
                            Stream filestream = new MemoryStream(bytes);
                            var sDocName = UploadPensionCorporateDocuments(filestream, fileName, sSpSiteRelativeUrl, Constants.SharepointLibrary, filedetails.policyNo, fileinfor, filedetails.fileExtension, 0);
                            ...
                            using (ClientContext ctx = new ClientContext(Constants.SharepointURL))
                            {
                                ...
                                try
                                {
                                    ...
                                    ctx.Load(ctx.Web);
                                    ctx.ExecuteQuery();
                                    var FolderRelativeURL = Constants.SharepointURL + Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sCorperate + "/" + filedetails.policyNo + "/" + fileName;
                                    Uri fileUri = new Uri(FolderRelativeURL);
                                    Microsoft.SharePoint.Client.File file = ctx.Web.GetFileByServerRelativeUrl(fileUri.AbsolutePath);
                                    ctx.Load(file);
                                    ctx.ExecuteQuery();
        ...
        /// </summary>
        [HttpGet]
        [Route("api/Portal/GetDocumentDetails/file_name")]
        ...
        public string UploadPensionPersonalDocuments(Stream fs, string sFileName, string sSpSiteRelativeUrl, string sLibraryName, string sDirectoryName, FileUploadInformation Document, string fileExtention, long fileSize)
        {
            string CorrectDirectoryName = sDirectoryName.Replace("\\", "/");
            var FinalDestination = Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sPersonal + "/" + CorrectDirectoryName;
            string parent_folderName = FinalDestination;
            var SharepointFileLink = Constants.SharepointURL + "/" + Constants.SharepointLibraryURI + "/" + parent_folderName + "/" + sFileName;
            var folderRelativeURL = sSpSiteRelativeUrl + "/" + parent_folderName;
        ...
        public string UploadPensionCorporateDocuments(Stream fs, string sFileName, string sSpSiteRelativeUrl, string sLibraryName, string sDirectoryName, FileUploadInformation Document, string fileExtention, long fileSize)
        {
            string CorrectDirectoryName = sDirectoryName.Replace("\\", "/");
            var FinalDestination = Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sCorperate + "/" + CorrectDirectoryName;
            string parent_folderName = FinalDestination;
            var SharepointFileLink = Constants.SharepointURL + "/" + Constants.SharepointLibraryURI + "/" + parent_folderName + "/" + sFileName;
            var folderRelativeURL = sSpSiteRelativeUrl + "/" + parent_folderName;
        ...
        /// </summary>
        [HttpDelete]
        [Route("api/Portal/Pension/Personal/DeletePolicyDocument/{policy_number}/{file_name}")]
        public IHttpActionResult DeletePensionPersonalDocumentByPolicy(string policy_number, string file_name)
        {
            ...
            if (!string.IsNullOrEmpty(filedetails.policyNo) && !string.IsNullOrEmpty(filedetails.fileName))
            {
                try
                {
                    ...
                    {
                        var secret = new SecureString();
                        var parentFolderName = Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sPersonal + "/" + filedetails.policyNo + "/";
                        foreach (char c in Constants.SharepointPassword)
                        {
                            secret.AppendChar(c);
                        }
        ...
        /// </summary>
        [HttpDelete]
        [Route("api/Portal/Pension/Corporate/DeletePolicyDocument/{policy_number}/{file_name}")]
        public IHttpActionResult DeletePensionCorporateDocumentByPolicy(string policy_number, string file_name)
        {
            ...
            if (!string.IsNullOrEmpty(filedetails.policyNo) && !string.IsNullOrEmpty(filedetails.fileName))
            {
                try
                {
                    ...
                    {
                        var secret = new SecureString();
                        var parentFolderName = Constants.SharepointLibraryURI + "/" + Constants.SPension + "/" + Constants.sPortal + "/" + Constants.sCorperate + "/" + filedetails.policyNo + "/";
                        foreach (char c in Constants.SharepointPassword)
                        {
                            secret.AppendChar(c);
                        }
        ...
        [HttpDelete]
        [Route("api/Portal/Pension/Personal/DeletePolicyDocument/{policy_number}/{file_name}")]
        public IHttpActionResult DeletePersonalDocumentByPolicy(string policy_number, string file_name)
        {
        ...
        [HttpDelete]
        [Route("api/Portal/Pension/Corporate/DeletePolicyDocument/{policy_number}/{file_name}")]
        public IHttpActionResult DeleteCorporateDocumentByPolicy(string policy_number, string file_name)
        {
        ...
        /// </summary>
        [HttpDelete]
        [Route("api/Portal/Portal/DeleteDocument/{file_name}")]
        public IHttpActionResult DeleteDocument(string file_name)
        {
            ...
            //string FileName = HttpContext.Current.Request.Params[0];
            string FileName = file_name;
            if (string.IsNullOrEmpty(FileName))
            {
                FileName = FileName.Replace('/', '_');
